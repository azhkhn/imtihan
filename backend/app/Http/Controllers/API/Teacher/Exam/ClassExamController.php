...
use App\Http\Requests\Teacher\Exam\StoreClassExamRequest;
use App\Http\Requests\Teacher\Exam\UpdateClassExamRequest;
use App\Http\Resources\Teacher\Exam\ClassExamResource;
...
class ClassExamController extends ApiController
{
    ...
    {
        abort_unless(auth()->user()->tokenCan('teacher.class-exam.list'),
    ...
    {
        abort_unless(auth()->user()->tokenCan('teacher.class-exam.create'),
            Response::HTTP_FORBIDDEN
        );
    ...
    {
        abort_unless(auth()->user()->tokenCan('teacher.class-exam.show'),
            Response::HTTP_FORBIDDEN
        );
    ...
    {
        abort_unless(auth()->user()->tokenCan('teacher.class-exam.update'),
            Response::HTTP_FORBIDDEN
        );
    ...
    {
        abort_unless(auth()->user()->tokenCan('teacher.class-exam.delete'),
            Response::HTTP_FORBIDDEN
        );
